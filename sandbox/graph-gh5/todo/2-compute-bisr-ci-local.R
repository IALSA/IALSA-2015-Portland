...
path_input      <- paste0(pathStudies,"/1-parsed-results.csv")
path_output     <- paste0(pathStudies,"/2-parsed-results-ci.csv")

## @knitr basic_view -----
keepvar <-  c("output_file", "subject_count","ab_TAU_00_est", "ab_TAU_11_est","ab_SIGMA_est", "a_GAMMA_00_est" )
ds <- read.csv(path_input, header = T,  stringsAsFactors=FALSE)
