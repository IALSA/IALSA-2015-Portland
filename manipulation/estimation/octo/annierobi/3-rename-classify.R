...
# ---- declare_globals ---------------------------------------------------------
path_input      <- "./data/shared/derived/dem-criteria/catalog-2.rds"
path_output     <- "./data/shared/derived/dem-criteria/catalog-3.rds"
...
ds0 <- readRDS(path_input) %>% tibble::as_tibble()
# define the list of variables containing the errors
error_vars <- grep("error_", unique(colnames(ds0)), value =T)

# ---- tweak_data --------------------------------------------------------------
colnames(ds0)
colnames(ds0) <- tolower(colnames(ds0))
# ds <- ds0 %>% dplyr::arrange_("model_type", "process_a") %>%
  # dplyr::select_("study_name", "model_number","subgroup","model_type","process_a", "process_b")
# ds <- ds0 %>%
#   dplyr::select_(.dots = c(model_id, model_info, "path"))

# ---- convert-to-catalog ----------------------
# list models with errors
d_error <- ds0 %>%
  dplyr::filter(error_count > 0L) %>%
  dplyr::distinct_(.dots = c(model_id, error_vars))

ds <- ds0 %>%
  dplyr::filter(error_count == 0L) %>% # use only those that converged
  dplyr::select_(.dots = c(model_id, model_info, model_spread)) %>%
  # dplyr::filter(is.na(name))
  # dplyr::filter(condition == "dem_ever_0") %>%
  # dplyr::filter(model_name %in% c(
  #   "b1_female_a_gait_block","b1_female_aeh_gait_mirrecog") ) %>%
  # dplyr::filter(model_name == "b1_female_aehplus_gait_block") %>%
  tidyr::gather_( "g","value",c("est","se","wald","pval") ) %>%
  dplyr::mutate(
    name = paste0(name,"_", g)
  ) %>%
  dplyr::select(-paramheader, -param, -effect, -label,
                # -est, -se, -wald, - pval,
                -est_pretty, -se_pretty, -pval_pretty, -dense,
                -g
                ) %>%
  tidyr::spread(name,value) %>%
  as.data.frame() %>%
  dplyr::full_join(d_error, by = model_id)
