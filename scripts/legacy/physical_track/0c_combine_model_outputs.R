...
######## load data #############################
## @knitr load_data
pattern <- "study_automation_result_physical.csv$"
...
study_names <- unique(out_files_all[[2]])
a <- strsplit(directories,split="/")
a %in% "studies"
...
saveRDS(ds,"./data/shared/ds0_physical.rds")
#
# source("./scripts/make_pretty.r")

# Export aggregated model results

destination <- file.path(path_input, "data/shared/physical_results.csv")
write.csv(ds, destination, row.names=F)
