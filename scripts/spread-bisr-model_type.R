...
# ---- remove-duplicates ---------------------
#
# define_duplicates <- c(variables_part_1, variables_part_2, "ll", "aic","bic")
define_duplicates <- c(variables_part_1, variables_part_2)
# define_duplicates <- names(ds_full)

ds_find_duplicates <- ds_full %>%
  dplyr::filter(
    study_name == "ilse"
  ) %>%
  dplyr::select_(.dots = define_duplicates)
temp <- ds_find_duplicates
...
# spread_bisr_model_type <- function()
  spread_dim = "process_b"
  ...
  # elongate (shape) data into the particular form
  # regex_correlation <- "^(a|b|aa|bb|ab|er|cr)_(\\w+)_(\\d{2})_(est|se|wald|pval|ci95_lower|ci95_upper)$"
  regex <- "^(a|b|aa|bb|ab|er|cr)_(\\w+)_(est|se|wald|pval|ci95_lower|ci95_upper)$"
  ...
  ds_target <- ds_full %>%
    ...
    dplyr::filter(model_number %in% c("b1")) %>% # only bivariate linear models
    dplyr::select_(
      .dots=c(
        variables_part_1  # id
    ...
    d <- tidyr::spread(temp,spread_names, value = value )
    d <- temp %>% dplyr::distinct_()
