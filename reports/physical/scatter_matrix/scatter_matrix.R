...
(a <- proto_scatter(dsL,x=i2, y=i1 ) + theme(legend.position="none") +
  ...
  # annotate(geom="text", size=baseSize-8, x=max_i2, y=min_i1, label=paste0(dsL$subgroup[1], "\n", "N = ",sample_N))+
...
(b <- proto_scatter(dsL,x=s2, y=s1)+
  annotate(geom="text", size=baseSize-6, x=max_s2, y=max_s1, label=R_SPSC_display, hjust=1)+
  # annotate(geom="text", size=baseSize-8, x=max_s2, y=min_s1, label=paste0(dsL$subgroup[1], "\n", "N = ",sample_N))+
