...
```{r select-phys-cog-full, echo=echoChunks, message=TRUE,eval=TRUE}
```

```{r select-phys-cog-short, echo=echoChunks, message=TRUE,eval=FALSE}
```
...
Raw covariance and estimated correlations are reported in the form : ` est(se) pval star`     
where      
...
```{r table-static-short, echo=echoChunks, message=TRUE, results='asis', eval=F}
```
