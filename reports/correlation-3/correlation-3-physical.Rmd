...
read_chunk("./reports/correlation-3/correlation-3-pp.R") #This allows knitr to call chunks tagged in the underlying *.R file.
```
...
```{r select-phys-phys, echo=echoChunks, results='show', message=FALSE, eval=F}
```

```{r select-phys-phys-full, echo=echoChunks, results='show', message=FALSE, eval=T}
```
...
```{r table-static-3, echo=echoChunks, message=TRUE, results='asis', eval=F}
```
